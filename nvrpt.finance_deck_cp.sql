...
from
  ...
  left join--customer id
        ...
        (select order_id, cust_grp_id
         from
            (select distinct order_id, cust_grp_id, rank() over (partition by order_id order by order_update_dt desc) as rank 
...
GRANT RULE, SELECT, INSERT, TRIGGER, UPDATE, DELETE, REFERENCES ON nvrpt.finance_deck_cp TO nvort_dev;
GRANT SELECT ON nvrpt.finance_deck_cp TO nvort_rs_etl;
